...
while True:
    if button.is_pressed(23):
        ...
        if stop_var == False and send == False:
            ...
            subprocess.Popen(["/bin/arecord", "-D", "plughw:CARD=Device,DEV=0", "-f", "S24_LE", "-r" "48000", "-t", "raw", "record.raw"])   # Запись файла
        else:
            pass
    ...
    if not(button.is_pressed(23)) and send == False:
        if not(recieve_file()):  # Если ничего нет в эфире, то пропускаем
            ...
            os.system("aplay -D plughw:CARD=Device,DEV=0 -f S24_LE -r 48000 -t raw sound.raw")  # Воспроизводим декодированный файл
